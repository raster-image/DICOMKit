...
final class PrivateCreatorTests: XCTestCase {
    ...
    func test_privateTag_generatesCorrectTag() {
        ...
        let tag1 = creator.privateTag(offset: 0x10)
        XCTAssertEqual(tag1?.group, 0x0009)
        XCTAssertEqual(tag1?.element, 0x0010) // block 0x00, offset 0x10
        
        let tag2 = creator.privateTag(offset: 0x20)
        XCTAssertEqual(tag2?.group, 0x0009)
        XCTAssertEqual(tag2?.element, 0x0020) // block 0x00, offset 0x20
        
        let tag3 = creator.privateTag(offset: 0xFF)
        XCTAssertEqual(tag3?.group, 0x0009)
        XCTAssertEqual(tag3?.element, 0x00FF) // block 0x00, offset 0xFF
    }
    ...
    func test_privateTag_withDifferentBlock() {
        ...
        XCTAssertEqual(tag?.group, 0x0009)
        XCTAssertEqual(tag?.element, 0x0110) // block 0x01, offset 0x10
    }
    
    func test_privateTag_invalidOffset_returnsNil() {
        let creator = PrivateCreator(creatorID: "TEST", group: 0x0009, element: 0x0010)
        
        XCTAssertNil(creator.privateTag(offset: 0x00))
        XCTAssertNil(creator.privateTag(offset: 0x0F))
    }
    ...
    func test_owns_correctlyIdentifiesOwnedTags() {
        ...
        XCTAssertTrue(creator.owns(Tag(group: 0x0009, element: 0x0010)))
        XCTAssertTrue(creator.owns(Tag(group: 0x0009, element: 0x0020)))
        XCTAssertTrue(creator.owns(Tag(group: 0x0009, element: 0x00FF)))
    }
    ...
    func test_owns_rejectsNonOwnedTags() {
        ...
        // Different group
        XCTAssertFalse(creator.owns(Tag(group: 0x000B, element: 0x0010)))
        
        // Different block
        XCTAssertFalse(creator.owns(Tag(group: 0x0009, element: 0x0110)))
        
        // Creator element itself is not owned
    ...
    func test_owns_withDifferentBlock() {
        ...
        XCTAssertTrue(creator.owns(Tag(group: 0x0009, element: 0x0210)))
        XCTAssertTrue(creator.owns(Tag(group: 0x0009, element: 0x0220)))
        XCTAssertFalse(creator.owns(Tag(group: 0x0009, element: 0x0110)))
    }
